...
def send_func(): # 로그인 기능을 send_func 함수에서 구현
    ...
    if state == 'working':
        ...
        f = open('test.txt', 'a')                #텍스트 파일 생성
        f.write(now_time()+ send_data+ '\n')     #현재 시간과 내용 저장
        f.close()                 #종료
def recv_func():
    global client_sock
    global state
...
ID_entry = Entry(window, width=16); ID_entry.place(x = 510, y=10)
pass_entry = Entry(window, width=16); pass_entry.place(x = 510, y=30)
